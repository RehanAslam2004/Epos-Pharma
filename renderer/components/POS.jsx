...
export default function POS() {
    ...
    const filtered = products.filter(p => {
        ...
        const matchSearch = !search || p.name?.toLowerCase().includes(s) || (p.barcode && p.barcode.toLowerCase().includes(s)) || (p.brand && p.brand.toLowerCase().includes(s));
    ...
    function addToCart(p) {
        ...
        if (idx >= 0) {
            ...
            setCart([...cart, { product_id: p.id, product_name: p.name, price: p.selling_price || 0, quantity: 1, max_stock: p.stock, batch: p.batch || '', category: p.category || 'General' }]);
        }
    }
    ...
    function clearCart() { setCart([]); setDiscount(0); setDiscountType('fixed'); setSelectedCustomer(null); setPayMethod('cash'); }

    const subtotal = cart.reduce((s, i) => s + (i.price * i.quantity), 0);
    const taxRate = settings?.tax_rate || 0;
    const taxAmt = subtotal * (taxRate / 100);
    const discountAmt = discountType === 'percent' ? subtotal * (Math.min(discount, 100) / 100) : Math.min(discount, subtotal);
    ...
    const completeSale = useCallback(async () => {
        ...
        try {
            const result = await api.createSale({
                ...
                discount: discountAmt, tax: taxAmt, payment_method: payMethod,
            });
        ...
        setProcessing(false);
    }, [cart, processing, selectedCustomer, discountAmt, taxAmt, payMethod, currency]);
        ...
        return (
            <>
                ...
                <div className="flex items-center justify-center h-full p-6 bg-gradient-to-br from-gray-50 via-white to-sea-50/30 dark:from-gray-900 dark:via-gray-900 dark:to-gray-800 print:hidden">
                    <div className="bg-white dark:bg-gray-800 rounded-3xl border border-gray-100 dark:border-gray-700 shadow-2xl shadow-sea-500/5 w-full max-w-[440px] animate-scale-up overflow-hidden">
                        ...
                        <div className="px-6 pb-5 pt-2 flex gap-3">
                            <button onClick={() => window.print()} className="flex-1 py-3 bg-white dark:bg-gray-700 border-2 border-sea-500 text-sea-700 dark:text-sea-400 font-bold rounded-xl text-sm hover:bg-sea-50 dark:hover:bg-sea-900/20 transition-all flex items-center justify-center gap-2">
                ...
                {/* ══════════════════════════════════════
                    ...
                    ══════════════════════════════════════ */}
                <div id="printable-receipt" className="hidden bg-white text-black p-4 z-[9999]">
                    <div style={{ fontFamily: "'Courier New', monospace", maxWidth: '320px', margin: '0 auto', fontSize: '12px' }}>
                        <h2 style={{ textAlign: 'center', fontSize: '18px', marginBottom: '2px', fontWeight: 'bold' }}>{pharmName}</h2>
                        {(pharmAddr || pharmPhone) && (
                            <p style={{ textAlign: 'center', fontSize: '11px', color: '#555', marginBottom: '10px' }}>
                                {pharmAddr}
                        ...
                        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                            <thead>
                                <tr style={{ fontSize: '11px', color: '#555', borderBottom: '1px solid #ddd' }}>
                            ...
                            <tbody>
                                {(sale.items || []).map((i, idx) => (
                                    <tr key={idx}>
                                        <td style={{ padding: '6px 0', fontSize: '13px', borderBottom: '1px solid #eee' }}>{i.product_name}</td>
                                        <td style={{ padding: '6px 4px', fontSize: '13px', textAlign: 'center', borderBottom: '1px solid #eee' }}>×{i.quantity}</td>
                                        <td style={{ padding: '6px 0', fontSize: '13px', textAlign: 'right', borderBottom: '1px solid #eee' }}>{currency} {(i.subtotal || i.price * i.quantity).toLocaleString()}</td>
                                    </tr>
                                ))}
                            </tbody>
                        </table>
                        ...
                        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                            <tbody>
                                <tr>
                                    <td style={{ fontSize: '13px', padding: '3px 0' }}>Subtotal</td>
                                    <td style={{ textAlign: 'right', fontSize: '13px', padding: '3px 0' }}>{currency} {(sale.subtotal || 0).toLocaleString()}</td>
                                </tr>
                                {(sale.discount || 0) > 0 && (
                                    <tr>
                                        <td style={{ fontSize: '13px', padding: '3px 0' }}>Discount</td>
                                        <td style={{ textAlign: 'right', fontSize: '13px', padding: '3px 0' }}>-{currency} {(sale.discount || 0).toLocaleString()}</td>
                                    </tr>
                                )}
                                {(sale.tax || 0) > 0 && (
                                    <tr>
                                        <td style={{ fontSize: '13px', padding: '3px 0' }}>Tax</td>
                                        <td style={{ textAlign: 'right', fontSize: '13px', padding: '3px 0' }}>{currency} {(sale.tax || 0).toLocaleString()}</td>
                                    </tr>
                                )}
                                <tr style={{ borderTop: '1px dashed #aaa' }}>
                                    <td style={{ fontSize: '16px', fontWeight: 'bold', paddingTop: '8px' }}>Total</td>
                                    <td style={{ textAlign: 'right', fontSize: '16px', fontWeight: 'bold', paddingTop: '8px' }}>{currency} {(sale.total || 0).toLocaleString()}</td>
                                </tr>
                            </tbody>
                        </table>
                        <hr style={{ border: 'none', borderTop: '1px dashed #aaa', margin: '10px 0' }} />
                        <p style={{ fontSize: '12px' }}>Payment: <strong style={{ textTransform: 'capitalize' }}>{sale.payment_method}</strong></p>
                        <p style={{ textAlign: 'center', fontSize: '10px', color: '#777', marginTop: '16px' }}>
                    ...
                    <div className="px-3 py-2 border-b border-gray-100 dark:border-gray-700">
                        {selectedCustomer ? (
                            <div className="flex items-center justify-between bg-sea-50 dark:bg-sea-900/20 text-sea-700 dark:text-sea-400 px-3 py-2 rounded-lg text-[11px] font-semibold">
                                ...
                                <button onClick={() => setSelectedCustomer(null)} className="text-sea-400 hover:text-sea-600 text-xs">✕</button>
                            </div>
                        ) : (
                            ...
                            {salesHistory.length === 0 ? (
                                ...
                                <table className="w-full">
                                    <thead><tr className="text-[10px] uppercase tracking-wider font-semibold text-gray-400 bg-gray-50 dark:bg-gray-800/50 border-b border-gray-100 dark:border-gray-700 sticky top-0">
                                        <th className="text-left px-5 py-3">#</th><th className="text-left px-5 py-3">Date</th><th className="text-left px-5 py-3">Customer</th><th className="text-left px-5 py-3">Payment</th><th className="text-right px-5 py-3">Total</th>
                                    </tr></thead>
                                    <tbody>{salesHistory.map(s => (
                                        <tr key={s.id} className="tbl-row border-b border-gray-50 dark:border-gray-700/50 last:border-b-0">
                                            <td className="px-5 py-3 text-xs text-gray-500 font-mono">#{s.id}</td>
                                            ...
                                            <td className="px-5 py-3 text-sm text-right font-bold text-sea-700 dark:text-sea-400">{currency} {(s.total || 0).toLocaleString()}</td>
                                        </tr>
                                    ))}</tbody>
                                </table>
                            )}

...
export default function Inventory() {
    ...
    const [form, setForm] = useState({ name: '', brand: '', batch: '', expiry: '', purchase_price: '', selling_price: '', stock: '', barcode: '', supplier_id: '', category: 'General', description: '' });
    ...
    async function handleSave(e) {
        ...
        try {
            const payload = { ...form, purchase_price: parseFloat(form.purchase_price) || 0, selling_price: parseFloat(form.selling_price) || 0, stock: parseInt(form.stock) || 0, supplier_id: form.supplier_id ? parseInt(form.supplier_id) : null };
                                    ...
                                    products.map(p => {
                                        ...
                                        return (
                                            <tr key={p.id} className={`tbl-row border-b border-gray-50 dark:border-gray-700/50 last:border-b-0 ${daysToExpiry !== null && daysToExpiry < 0 ? 'bg-red-50/50 dark:bg-red-900/10' : daysToExpiry !== null && daysToExpiry <= 30 ? 'bg-amber-50/50 dark:bg-amber-900/10' : ''}`}>
                                                <td className="px-4 py-3 text-sm font-semibold text-gray-800 dark:text-gray-200">{p.name}</td>
                        ...
                        <form onSubmit={handleSave}>
                            <div className="px-6 py-5 space-y-4">
                                ...
                                <div className="grid grid-cols-3 gap-4">
                                    <div><label className="block text-[11px] font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide mb-1">Batch</label><input className={inputCls} value={form.batch} onChange={e => setForm({ ...form, batch: e.target.value })} /></div>
                                    <div><label className="block text-[11px] font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide mb-1">Expiry</label><input className={inputCls} type="date" value={form.expiry} onChange={e => setForm({ ...form, expiry: e.target.value })} /></div>
